...
struct Camera {
  ...
  bool firstClick;
  double lastMouseX, lastMouseY; // for delta-based mouse movement (browser-friendly)
};

Camera CreateOrbitCamera(glm::vec3 target, float distance, int width,
                         int height);
void SendCameraMatrix(Camera *camera, GLuint shaderID, const char *uniform,
                      float t);
