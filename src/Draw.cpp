...
void DrawText(Renderer *renderer, Camera *cam, const char *text, glm::vec3 pos,
              float size, glm::u8vec4 color) {
  if (color.a == 255)
    color.a = 254; // make sure we use transparent path
  ...
  for (const char *s = text; *s; s++) {
    ...
    glm::vec3 v1 =
        pos + (cam->camRight * x1_local) + (cam->camUp * y1_local); // Top-Left
    glm::vec3 v2 =
        pos + (cam->camRight * x2_local) + (cam->camUp * y1_local); // Top-Right
    glm::vec3 v3 =
        pos + (cam->camRight * x2_local) + (cam->camUp * y2_local); // Bot-Right
    glm::vec3 v4 =
        pos + (cam->camRight * x1_local) + (cam->camUp * y2_local); // Bot-Left
    float U1 = glyph->U0;
    float V1 = glyph->V0;
    float U2 = glyph->U1;
    float V2 = glyph->V1;
    Renderer_PushVertex(renderer, {v1.x, v1.y, v1.z, cam->camRight.x,
                                   cam->camRight.y, cam->camRight.z, U1, V1,
                                   color.r, color.g, color.b, color.a});
    Renderer_PushVertex(renderer, {v2.x, v2.y, v2.z, cam->camRight.x,
                                   cam->camRight.y, cam->camRight.z, U2, V1,
                                   color.r, color.g, color.b, color.a});
    Renderer_PushVertex(renderer, {v3.x, v3.y, v3.z, cam->camRight.x,
                                   cam->camRight.y, cam->camRight.z, U2, V2,
                                   color.r, color.g, color.b, color.a});
    Renderer_PushVertex(renderer, {v1.x, v1.y, v1.z, cam->camRight.x,
                                   cam->camRight.y, cam->camRight.z, U1, V1,
                                   color.r, color.g, color.b, color.a});
    Renderer_PushVertex(renderer, {v3.x, v3.y, v3.z, cam->camRight.x,
                                   cam->camRight.y, cam->camRight.z, U2, V2,
                                   color.r, color.g, color.b, color.a});
    Renderer_PushVertex(renderer, {v4.x, v4.y, v4.z, cam->camRight.x,
                                   cam->camRight.y, cam->camRight.z, U1, V2,
                                   color.r, color.g, color.b, color.a});
...
// Helper to find intersection between a line (Point + Dir) and a Plane (Point +
// Normal) Based on the formula: t = ((PlanePoint - LineStart) . PlaneNormal) /
// (LineDir . PlaneNormal)
static glm::vec3 IntersectLinePlane(glm::vec3 lineStart, glm::vec3 lineDir,
                                    glm::vec3 planePoint,
...
static void DrawCap(Renderer *renderer, glm::vec3 center,
                    const std::vector<PipeNode> &ring, glm::vec3 normal,
                    glm::u8vec4 color, bool isStartCap) {
  ...
  //     // If it's a start cap, we look at it from behind, so we swap winding
  //     order
  ...
  //     Renderer_PushVertex(renderer, { vCenter.x, vCenter.y, vCenter.z,
  //     normal.x, normal.y, normal.z, white.x, white.y, color.r, color.g,
  //     color.b, color.a }); Renderer_PushVertex(renderer, { vCurrent.x,
  //     vCurrent.y, vCurrent.z, normal.x, normal.y, normal.z, white.x, white.y,
  //     color.r, color.g, color.b, color.a }); Renderer_PushVertex(renderer, {
  //     vNext.x, vNext.y, vNext.z, normal.x, normal.y, normal.z, white.x,
  //     white.y, color.r, color.g, color.b, color.a });
  // }
  return;
}

static void DrawHiddenSegment(Renderer *renderer, glm::vec3 *pos, int posSize,
                              float radius, glm::u8vec4 color) {
  if (posSize < 2 || pos == nullptr || renderer == nullptr)
    return;
  ...
  for (int i = 1; i < posSize; i++) {
    ...
    for (int j = 0; j < SEGMENTS; j++) {
      ...
      // Math from image:
      // Find P' (currRing pos) by intersecting the line from P_prev with
      // direction v1 against the plane defined by (Point: Q_curr, Normal:
      // planeNormal).

      currRing[j].position =
          IntersectLinePlane(P_prev, v1, Q_curr, planeNormal);
    ...
    for (int j = 0; j < SEGMENTS; j++) {
      ...
      Renderer_PushHiddenVertex(renderer,
                                {p0.position.x, p0.position.y, p0.position.z,
                                 p0.normal.x, p0.normal.y, p0.normal.z, white.x,
                                 white.y, color.r, color.g, color.b, color.a});
      Renderer_PushHiddenVertex(renderer,
                                {c0.position.x, c0.position.y, c0.position.z,
                                 c0.normal.x, c0.normal.y, c0.normal.z, white.x,
                                 white.y, color.r, color.g, color.b, color.a});
      Renderer_PushHiddenVertex(renderer,
                                {p1.position.x, p1.position.y, p1.position.z,
                                 p1.normal.x, p1.normal.y, p1.normal.z, white.x,
                                 white.y, color.r, color.g, color.b, color.a});
      ...
      Renderer_PushHiddenVertex(renderer,
                                {c1.position.x, c1.position.y, c1.position.z,
                                 c1.normal.x, c1.normal.y, c1.normal.z, white.x,
                                 white.y, color.r, color.g, color.b, color.a});
    }
    if (i == posSize - 1) {
...
void DrawSegment(Renderer *renderer, glm::vec3 *pos, int posSize, float radius,
                 glm::u8vec4 color, float dashLen, float gapLen) {
  if (posSize < 2 || !pos || !renderer)
    return;
  if (color.a == 0)
    return;
  if (dashLen <= 0.0f)
  ...
  for (int i = 0; i < posSize - 1; i++) {
    ...
    glm::vec3 globalUp =
        (glm::abs(dir.y) > 0.99f) ? glm::vec3(0, 0, 1) : glm::vec3(0, 1, 0);
    ...
    while (distTraveled < segLen) {
      ...
      float distToNextState = isDash ? (dashLen - currentPatternPos)
                                     : (patternLen - currentPatternPos);
      ...
      if (isDash) {
        ...
        for (int j = 0; j < SEGMENTS; j++) {
          glm::vec3 offset =
              (right * circleLUT[j].x + up * circleLUT[j].y) * radius;
          glm::vec3 normal =
              glm::normalize(offset); // Normal points out from center
        ...
        for (int j = 0; j < SEGMENTS; j++) {
          ...
          PipeNode &s0 = startRing[j];
          PipeNode &s1 = startRing[next];
          PipeNode &e0 = endRing[j];
          PipeNode &e1 = endRing[next];

          Renderer_PushVertex(renderer,
                              {s0.position.x, s0.position.y, s0.position.z,
                               s0.normal.x, s0.normal.y, s0.normal.z, uv.x,
                               uv.y, color.r, color.g, color.b, color.a});
          Renderer_PushVertex(renderer,
                              {e0.position.x, e0.position.y, e0.position.z,
                               e0.normal.x, e0.normal.y, e0.normal.z, uv.x,
                               uv.y, color.r, color.g, color.b, color.a});
          Renderer_PushVertex(renderer,
                              {s1.position.x, s1.position.y, s1.position.z,
                               s1.normal.x, s1.normal.y, s1.normal.z, uv.x,
                               uv.y, color.r, color.g, color.b, color.a});
          ...
          Renderer_PushVertex(renderer,
                              {e1.position.x, e1.position.y, e1.position.z,
                               e1.normal.x, e1.normal.y, e1.normal.z, uv.x,
                               uv.y, color.r, color.g, color.b, color.a});
        }
      ...
      if (currentPatternPos >= patternLen)
        currentPatternPos -= patternLen;
    }
  }
}

void DrawAngle(Renderer *renderer, const Camera *cam, glm::vec3 start,
               glm::vec3 vertex, glm::vec3 end, glm::u8vec4 color, float radius,
               float thickness, float textSize, float textOffset) {
  if (!renderer || !cam)
    return;
  ...
  if (glm::length(v1) < 0.001f || glm::length(v2) < 0.001f)
    return;
  ...
  if (std::abs(angleDeg - 90.0f) < 0.5f) {
    ...
    // We need a smaller radius for the square symbol usually, or use the
    // provided radius Let's use the provided radius directly.
    ...
    DrawSegment(renderer, arcPoints.data(), (int)arcPoints.size(), thickness,
                color, 0.0f, 0.0f);
  }
...
void DrawLine(Renderer *renderer, glm::vec3 p1, glm::vec3 p2, glm::u8vec4 color,
...
void DrawSurface(Renderer *renderer, glm::vec3 *points, int count,
                 glm::u8vec4 surfaceColor, glm::u8vec4 borderColor,
                 float thickness, float dashLen, float gapLen) {
  ...
  if (isValidSurface && 1) {
    ...
    if (glm::abs(avgNormal.y) > maxN) {
      maxN = glm::abs(avgNormal.y);
      dropAxis = 1;
    }
    if (glm::abs(avgNormal.z) > maxN) {
      maxN = glm::abs(avgNormal.z);
      dropAxis = 2;
    }
    ...
    for (int i = 0; i < count; i++) {
      if (dropAxis == 0)
        poly2D[i] = {points[i].y, points[i].z};
      else if (dropAxis == 1)
        poly2D[i] = {points[i].z, points[i].x};
      else
        poly2D[i] = {points[i].x, points[i].y};
    }
      ...
      for (size_t i = 0; i < n; i++) {
        ...
        if (isConvex) {
          ...
          for (size_t j = 0; j < n; j++) {
            int other = indices[j];
            if (other == iPrev || other == iCurr || other == iNext)
              continue;
          ...
          if (empty) {
            ...
            if (glm::length(triNormal) > 1e-6f) {
              ...
              // Front
              Renderer_PushVertex(
                  renderer, {p0.x, p0.y, p0.z, triNormal.x, triNormal.y,
                             triNormal.z, uv.x, uv.y, surfaceColor.r,
                             surfaceColor.g, surfaceColor.b, surfaceColor.a});
              Renderer_PushVertex(
                  renderer, {p1.x, p1.y, p1.z, triNormal.x, triNormal.y,
                             triNormal.z, uv.x, uv.y, surfaceColor.r,
                             surfaceColor.g, surfaceColor.b, surfaceColor.a});
              Renderer_PushVertex(
                  renderer, {p2.x, p2.y, p2.z, triNormal.x, triNormal.y,
                             triNormal.z, uv.x, uv.y, surfaceColor.r,
                             surfaceColor.g, surfaceColor.b, surfaceColor.a});
              ...
              Renderer_PushVertex(
                  renderer, {p0.x, p0.y, p0.z, invNormal.x, invNormal.y,
                             invNormal.z, uv.x, uv.y, surfaceColor.r,
                             surfaceColor.g, surfaceColor.b, surfaceColor.a});
              Renderer_PushVertex(
                  renderer, {p2.x, p2.y, p2.z, invNormal.x, invNormal.y,
                             invNormal.z, uv.x, uv.y, surfaceColor.r,
                             surfaceColor.g, surfaceColor.b, surfaceColor.a});
              Renderer_PushVertex(
                  renderer, {p1.x, p1.y, p1.z, invNormal.x, invNormal.y,
                             invNormal.z, uv.x, uv.y, surfaceColor.r,
                             surfaceColor.g, surfaceColor.b, surfaceColor.a});
            }
  ...
  if (borderColor.a > 0 && thickness > 0.0f) {
    for (int i = 0; i < count; i++) {
      ...
      // This ensures adjacent faces draw the dash pattern starting from the
      // same vertex.
      bool swap = false;
      if (p1.x > p2.x)
        swap = true;
...
void DrawPlane(Renderer *renderer, glm::vec3 p1, glm::vec3 p2, glm::vec3 p3,
               glm::u8vec4 color) {
  if (!renderer)
    return;
  ...
  if (len < 1e-5f)
    return; // Points are collinear, cannot define a plane
  normal /= len;
  ...
  // Triangle 1: c1 -> c2 -> c3
  Renderer_PushVertex(renderer,
                      {c1.x, c1.y, c1.z, normal.x, normal.y, normal.z, uv.x,
                       uv.y, color.r, color.g, color.b, color.a});
  Renderer_PushVertex(renderer,
                      {c2.x, c2.y, c2.z, normal.x, normal.y, normal.z, uv.x,
                       uv.y, color.r, color.g, color.b, color.a});
  Renderer_PushVertex(renderer,
                      {c3.x, c3.y, c3.z, normal.x, normal.y, normal.z, uv.x,
                       uv.y, color.r, color.g, color.b, color.a});
  ...
  Renderer_PushVertex(renderer,
                      {c3.x, c3.y, c3.z, normal.x, normal.y, normal.z, uv.x,
                       uv.y, color.r, color.g, color.b, color.a});
  Renderer_PushVertex(renderer,
                      {c4.x, c4.y, c4.z, normal.x, normal.y, normal.z, uv.x,
                       uv.y, color.r, color.g, color.b, color.a});
}

void DrawCircle(Renderer *renderer, glm::vec3 midpoint, glm::vec3 edgepoint,
                glm::vec3 normpoint, int steps,
                glm::u8vec4 surfaceColor, // Fill Style
                glm::u8vec4 borderColor, float thickness, float dashLen,
                float gapLen) // Border Style
{
  if (steps < 3)
    steps = 36;
  if (!renderer)
  ...
  if (glm::length(vectorV) < 0.001f) {
    glm::vec3 arbitrary =
        (glm::abs(vectorU.y) > 0.99f) ? glm::vec3(1, 0, 0) : glm::vec3(0, 1, 0);
  ...
  for (int i = 0; i <= steps; i++) {
    ...
    points.push_back(midpoint + (vectorU * cos(theta)) +
                     (vectorV * sin(theta)));
  }
  ...
  if (1) {
    ...
    for (int i = 0; i < steps; i++) {
      ...
      Renderer_PushVertex(renderer,
                          {midpoint.x, midpoint.y, midpoint.z, surfaceNormal.x,
                           surfaceNormal.y, surfaceNormal.z, uv.x, uv.y,
                           surfaceColor.r, surfaceColor.g, surfaceColor.b,
                           surfaceColor.a});
      Renderer_PushVertex(renderer,
                          {p1.x, p1.y, p1.z, surfaceNormal.x, surfaceNormal.y,
                           surfaceNormal.z, uv.x, uv.y, surfaceColor.r,
                           surfaceColor.g, surfaceColor.b, surfaceColor.a});
      Renderer_PushVertex(renderer,
                          {p2.x, p2.y, p2.z, surfaceNormal.x, surfaceNormal.y,
                           surfaceNormal.z, uv.x, uv.y, surfaceColor.r,
                           surfaceColor.g, surfaceColor.b, surfaceColor.a});
    }
  }
  ...
  // 4. Draw Border
  if (borderColor.a > 0 && thickness > 0.0f) {
    DrawSegment(renderer, points.data(), (int)points.size(), thickness,
                borderColor, dashLen, gapLen);
  }
}

void DrawSphere(Renderer *renderer, const Camera *cam, glm::vec3 center,
                glm::vec3 radiusPoint, int sectorCount, int stackCount,
                glm::u8vec4 sphereColor, glm::u8vec4 ringColor,
                float ringThickness, float ringDashLen, float ringGapLen) {
  if (!renderer)
    return;
  ...
  if (1) {
    for (int i = 0; i < stackCount; ++i) {
      ...
      for (int j = 0; j < sectorCount; ++j) {
        ...
        // FIX: Swapped vertex order to ensure Counter-Clockwise winding
        // (Outward Facing)
  ...
  if (cam && ringColor.a > 0 && ringThickness > 0.0f) {
    ...
    glm::vec3 arbitraryUp =
        (glm::abs(viewDir.y) > 0.99f) ? glm::vec3(1, 0, 0) : glm::vec3(0, 1, 0);
    ...
    DrawSegment(renderer, ringPoints.data(), (int)ringPoints.size(),
                ringThickness, ringColor, ringDashLen, ringGapLen);
  }
}

void DrawCylinder(Renderer *renderer, const Camera *cam, glm::vec3 bottomCenter,
                  glm::vec3 topCenter, glm::vec3 bottomEdge, int sectorCount,
                  glm::u8vec4 cylinderColor, glm::u8vec4 outlineColor,
                  float outlineThickness, float dashLen, float gapLen) {
  if (!renderer || sectorCount < 3)
    return;
  ...
  if (1) {
    for (int i = 0; i < sectorCount; ++i) {
      ...
      // Side Wall
      Renderer_PushVertex(renderer, {b1.x, b1.y, b1.z, n1.x, n1.y, n1.z, uv.x,
                                     uv.y, cylinderColor.r, cylinderColor.g,
                                     cylinderColor.b, cylinderColor.a});
      Renderer_PushVertex(renderer, {b2.x, b2.y, b2.z, n2.x, n2.y, n2.z, uv.x,
                                     uv.y, cylinderColor.r, cylinderColor.g,
                                     cylinderColor.b, cylinderColor.a});
      Renderer_PushVertex(renderer, {t1.x, t1.y, t1.z, n1.x, n1.y, n1.z, uv.x,
                                     uv.y, cylinderColor.r, cylinderColor.g,
                                     cylinderColor.b, cylinderColor.a});
      ...
      Renderer_PushVertex(renderer, {b2.x, b2.y, b2.z, n2.x, n2.y, n2.z, uv.x,
                                     uv.y, cylinderColor.r, cylinderColor.g,
                                     cylinderColor.b, cylinderColor.a});
      Renderer_PushVertex(renderer, {t2.x, t2.y, t2.z, n2.x, n2.y, n2.z, uv.x,
                                     uv.y, cylinderColor.r, cylinderColor.g,
                                     cylinderColor.b, cylinderColor.a});
      ...
      Renderer_PushVertex(renderer,
                          {bottomCenter.x, bottomCenter.y, bottomCenter.z,
                           botN.x, botN.y, botN.z, uv.x, uv.y, cylinderColor.r,
                           cylinderColor.g, cylinderColor.b, cylinderColor.a});
      Renderer_PushVertex(renderer,
                          {b2.x, b2.y, b2.z, botN.x, botN.y, botN.z, uv.x, uv.y,
                           cylinderColor.r, cylinderColor.g, cylinderColor.b,
                           cylinderColor.a});
      Renderer_PushVertex(renderer,
                          {b1.x, b1.y, b1.z, botN.x, botN.y, botN.z, uv.x, uv.y,
                           cylinderColor.r, cylinderColor.g, cylinderColor.b,
                           cylinderColor.a});

      // Top Cap
      Renderer_PushVertex(renderer,
                          {topCenter.x, topCenter.y, topCenter.z, topN.x,
                           topN.y, topN.z, uv.x, uv.y, cylinderColor.r,
                           cylinderColor.g, cylinderColor.b, cylinderColor.a});
      Renderer_PushVertex(renderer,
                          {t1.x, t1.y, t1.z, topN.x, topN.y, topN.z, uv.x, uv.y,
                           cylinderColor.r, cylinderColor.g, cylinderColor.b,
                           cylinderColor.a});
      Renderer_PushVertex(renderer,
                          {t2.x, t2.y, t2.z, topN.x, topN.y, topN.z, uv.x, uv.y,
                           cylinderColor.r, cylinderColor.g, cylinderColor.b,
                           cylinderColor.a});
    }
  }
  ...
  if (cam && outlineColor.a > 0 && outlineThickness > 0.0f) {
    ...
    DrawSegment(renderer, botPoints.data(), (int)botPoints.size(),
                outlineThickness, outlineColor, dashLen, gapLen);
    DrawSegment(renderer, topPoints.data(), (int)topPoints.size(),
                outlineThickness, outlineColor, dashLen, gapLen);
    ...
    DrawSegment(renderer, line1, 2, outlineThickness, outlineColor, dashLen,
                gapLen);
    DrawSegment(renderer, line2, 2, outlineThickness, outlineColor, dashLen,
                gapLen);
  }
}

void DrawCone(Renderer *renderer, const Camera *cam, glm::vec3 bottomCenter,
              glm::vec3 topCenter, glm::vec3 bottomEdge, int sectorCount,
              glm::u8vec4 coneColor, glm::u8vec4 outlineColor,
              float outlineThickness, float dashLen, float gapLen) {
  if (!renderer || sectorCount < 3)
    return;
  ...
  if (1) {
    for (int i = 0; i < sectorCount; ++i) {
      ...
      // Side Triangle
      Renderer_PushVertex(renderer,
                          {topCenter.x, topCenter.y, topCenter.z, sideN1.x,
                           sideN1.y, sideN1.z, uv.x, uv.y, coneColor.r,
                           coneColor.g, coneColor.b, coneColor.a});
      Renderer_PushVertex(renderer, {b1.x, b1.y, b1.z, sideN1.x, sideN1.y,
                                     sideN1.z, uv.x, uv.y, coneColor.r,
                                     coneColor.g, coneColor.b, coneColor.a});
      Renderer_PushVertex(renderer, {b2.x, b2.y, b2.z, sideN2.x, sideN2.y,
                                     sideN2.z, uv.x, uv.y, coneColor.r,
                                     coneColor.g, coneColor.b, coneColor.a});

      // Base Cap Triangle
      Renderer_PushVertex(renderer,
                          {bottomCenter.x, bottomCenter.y, bottomCenter.z,
                           botN.x, botN.y, botN.z, uv.x, uv.y, coneColor.r,
                           coneColor.g, coneColor.b, coneColor.a});
      Renderer_PushVertex(renderer,
                          {b2.x, b2.y, b2.z, botN.x, botN.y, botN.z, uv.x, uv.y,
  ...
  if (cam && outlineColor.a > 0 && outlineThickness > 0.0f) {
    ...
    // Project onto plane perpendicular to axis
    glm::vec3 projCamVec =
        camToBottom - (glm::dot(camToBottom, axisDir) * axisDir);
...
void RecalculatePoint(DrawEntities &entities, int idx) {
  if (idx < 0 || idx >= (int)entities.points.size())
    return;
  Point &p = entities.points[idx];

  auto ParentPos = [&](int pIdx) -> glm::vec3 {
    if (pIdx >= 0 && pIdx < (int)entities.points.size())
      return entities.points[pIdx].pos;
...
void PropagateUpdates(DrawEntities &entities, int startNodeIdx) {
  if (startNodeIdx < 0 || startNodeIdx >= (int)entities.points.size())
    return;
...
void SetPointParent(DrawEntities &entities, int childIdx, int parentSlot,
                    int newParentIdx) {
  if (childIdx < 0 || childIdx >= (int)entities.points.size())
    return;
  Point &child = entities.points[childIdx];
  if (parentSlot < 0 || parentSlot >= (int)child.parents.size())
    return;
  ...
  if (oldParentIdx >= 0 && oldParentIdx < (int)entities.points.size()) {
    ...
    siblings.erase(std::remove(siblings.begin(), siblings.end(), childIdx),
                   siblings.end());
  }
...
int CreatePoint(DrawEntities &entities, Operation op,
                const std::vector<int> &parents, float param = 0.0f) {
  Point p;
...
static bool PointSelector(const char *label, int *currentIdx,
                          const std::vector<Point> &points) {
  if (points.empty()) {
    ImGui::Text("%s: No Pts", label);
    return false;
  }
  if (*currentIdx < 0)
    *currentIdx = 0;
  if (*currentIdx >= (int)points.size())
    *currentIdx = (int)points.size() - 1;

  std::string preview =
      std::to_string(*currentIdx) + ": " + points[*currentIdx].name;
  bool changed = false;
...
static void DeletePoint(DrawEntities &entities, int indexToDelete) {
  if (indexToDelete < 0 || indexToDelete >= (int)entities.points.size())
    return;
  ...
  if (!entities.points[indexToDelete].children.empty()) {
    for (int child : entities.points[indexToDelete].children) {
      ...
      parents.erase(std::remove(parents.begin(), parents.end(), indexToDelete),
                    parents.end());
  ...
  for (int parent : entities.points[indexToDelete].parents) {
    ...
    siblings.erase(std::remove(siblings.begin(), siblings.end(), indexToDelete),
                   siblings.end());
  }
  ...
  for (auto &p : entities.points) {
    for (int &pid : p.parents)
      FixIdx(pid);
    for (int &cid : p.children)
      FixIdx(cid);
  }
  for (auto &l : entities.lines) {
    FixIdx(l.point1);
    FixIdx(l.point2);
  }
  for (auto &r : entities.rays) {
    FixIdx(r.point1);
    FixIdx(r.point2);
  }
  for (auto &s : entities.segments) {
    FixIdx(s.point1);
    FixIdx(s.point2);
  }
  for (auto &surf : entities.surfaces) {
    for (int &pid : surf.pointIndices)
      FixIdx(pid);
  }
  for (auto &p : entities.planes) {
    FixIdx(p.point1);
    FixIdx(p.point2);
    FixIdx(p.point3);
  }
  for (auto &c : entities.circles) {
    FixIdx(c.midpoint);
    FixIdx(c.edgepoint);
    FixIdx(c.normpoint);
  }
  for (auto &s : entities.spheres) {
    FixIdx(s.midpoint);
    FixIdx(s.edgepoint);
  }
  for (auto &c : entities.cylinders) {
    FixIdx(c.midpoint);
    FixIdx(c.edgepoint);
    FixIdx(c.toppoint);
  }
  for (auto &c : entities.cones) {
    FixIdx(c.midpoint);
    FixIdx(c.edgepoint);
    FixIdx(c.toppoint);
  }
  for (auto &a : entities.angles) {
    FixIdx(a.p1);
    FixIdx(a.vertex);
    FixIdx(a.p2);
  }
...
void SaveScene(const DrawEntities &entities, const std::string &filename) {
#ifdef __EMSCRIPTEN__
  // For web: save to a buffer and trigger download
  ...
  out << "GLOBAL " << entities.globalTextSize << " "
      << entities.globalTextOffset << "\n";

  // 1. Points
  out << "POINTS " << entities.points.size() << "\n";
  for (const auto &p : entities.points) {
    // Format: Name Op Param VisualRadius Fixed ShowLabel
    // To simplify name saving, we replace spaces with underscores or assume
    // single word. For this example, we assume name is safe or quote it.
  ...
  for (const auto &l : entities.lines) {
    out << l.point1 << " " << l.point2 << " " << l.thickness << " " << l.dashLen
        << " " << l.gapLen << " " << l.color << "\n";
  }
  ...
  for (const auto &s : entities.segments) {
    out << s.point1 << " " << s.point2 << " " << s.thickness << " " << s.dashLen
        << " " << s.gapLen << " " << s.color << "\n";
  }
  ...
  for (const auto &r : entities.rays) {
    out << r.point1 << " " << r.point2 << " " << r.thickness << " " << r.dashLen
        << " " << r.gapLen << " " << r.color << "\n";
  }
  ...
  for (const auto &a : entities.angles) {
    out << a.p1 << " " << a.vertex << " " << a.p2 << " " << a.radius << " "
        << a.thickness << " " << a.color << "\n";
  }
  ...
  for (const auto &c : entities.circles) {
    out << c.midpoint << " " << c.edgepoint << " " << c.normpoint << " "
        << c.steps << " " << c.thickness << " " << c.dashLen << " " << c.gapLen
  ...
  for (const auto &s : entities.surfaces) {
    out << s.surfaceColor << " " << s.borderColor << " " << s.borderThickness
        << " " << s.dashLen << " " << s.gapLen << "\n";
    out << s.pointIndices.size();
    for (int idx : s.pointIndices)
      out << " " << idx;
    out << "\n";
  }
  ...
  for (const auto &s : entities.spheres) {
    out << s.midpoint << " " << s.edgepoint << " " << s.sectorCount << " "
        << s.stackCount << " " << s.ringThickness << " " << s.dashLen << " "
        << s.gapLen << " " << s.sphereColor << " " << s.ringColor << "\n";
  }

  // 10. Cylinders
  out << "CYLINDERS " << entities.cylinders.size() << "\n";
  for (const auto &c : entities.cylinders) {
    out << c.midpoint << " " << c.edgepoint << " " << c.toppoint << " "
        << c.sectorCount << " " << c.outlineThickness << " " << c.dashLen << " "
        << c.gapLen << " " << c.cylinderColor << " " << c.outlineColor << "\n";
  }
  ...
  for (const auto &c : entities.cones) {
    out << c.midpoint << " " << c.edgepoint << " " << c.toppoint << " "
        << c.sectorCount << " " << c.outlineThickness << " " << c.dashLen << " "
        << c.gapLen << " " << c.coneColor << " " << c.outlineColor << "\n";
  }
  ...
  EM_ASM_(
      {
        ...
        link.href = url;
        link.download = 'scene_' + new Date().getTime() + '.txt';
...
// Helper function to load scene from any input stream
void LoadSceneFromStream(DrawEntities &entities, std::istream &in) {
  // Clear existing
  ...
  auto CheckHeader = [&](const std::string &expected) {
    std::string h;
    // Peek or read until we find header? Simple approach: assume strict order
    // for now
    in >> h;
    if (h != expected)
      std::cerr << "Warning: Expected " << expected << " got " << h
                << std::endl;
    in >> count;
  };
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    int opInt;
    in >> p.name >> opInt >> p.param >> p.visualRadius >> p.fixed >>
        p.showLabel;
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    in >> l.point1 >> l.point2 >> l.thickness >> l.dashLen >> l.gapLen >>
        l.color;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    in >> s.point1 >> s.point2 >> s.thickness >> s.dashLen >> s.gapLen >>
        s.color;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    in >> r.point1 >> r.point2 >> r.thickness >> r.dashLen >> r.gapLen >>
        r.color;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    in >> c.midpoint >> c.edgepoint >> c.normpoint >> c.steps >> c.thickness >>
        c.dashLen >> c.gapLen >> c.surfaceColor >> c.borderColor;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    in >> s.surfaceColor >> s.borderColor >> s.borderThickness >> s.dashLen >>
        s.gapLen;
    size_t idxCount;
    in >> idxCount;
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    in >> s.midpoint >> s.edgepoint >> s.sectorCount >> s.stackCount >>
        s.ringThickness >> s.dashLen >> s.gapLen >> s.sphereColor >>
        s.ringColor;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    in >> c.midpoint >> c.edgepoint >> c.toppoint >> c.sectorCount >>
        c.outlineThickness >> c.dashLen >> c.gapLen >> c.cylinderColor >>
        c.outlineColor;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    in >> c.midpoint >> c.edgepoint >> c.toppoint >> c.sectorCount >>
        c.outlineThickness >> c.dashLen >> c.gapLen >> c.coneColor >>
        c.outlineColor;
  }
}
...
void LoadScene(DrawEntities &entities, const std::string &filename) {
  ...
  LoadSceneFromStream(entities, in);
}

// Web file loading wrapper - loads scene from string content
#ifdef __EMSCRIPTEN__
extern "C" {
static DrawEntities *g_webScenePtr = nullptr;

EMSCRIPTEN_KEEPALIVE void loadSceneFromWebFile(const char *content) {
  if (!g_webScenePtr || !content)
    return;

  try {
    std::istringstream ss{content};
    LoadSceneFromStream(*g_webScenePtr, ss);
    ErrLog("Scene loaded from file successfully");
  } catch (const std::exception &e) {
    ErrLog("Error loading scene: %s", e.what());
  }
}

EMSCRIPTEN_KEEPALIVE void setWebScenePtr(void *ptr) {
  g_webScenePtr = (DrawEntities *)ptr;
}
}
#endif
...
#ifdef __EMSCRIPTEN__
  EM_ASM_(
      { Module.ccall('setWebScenePtr', null, ['number'], [$0]); },
      (intptr_t)entities);
#endif
}
...
void DrawGeometryEditor(DrawEntities &entities) {
  ...
  if (ImGui::CollapsingHeader("File Operations",
    ...
    if (ImGui::Button("Save Scene")) {
      SaveScene(entities, "scene.txt");
    }

    ImGui::SameLine();

    // LOAD (for web, opens file picker; desktop loads from file)
      ...
      EM_ASM({
        ...
        input.onchange = function(e) {
          ...
          reader.onload = function(event) {
            ...
            Module.ccall('loadSceneFromWebFile', null, ['string'], [content]);
          };
...
#else
      // Desktop: check if scene is empty
      if (entities.points.empty()) {
        LoadScene(entities, "scene.txt");
      } else {
    ...
    if (ImGui::Button("Clear Scene")) {
      if (!entities.points.empty()) {
        ...
        entities = DrawEntities();
#else
        // Desktop: show confirmation popup
...
// --- POPUP MODAL LOGIC (Desktop only) ---
  ...
  if (ImGui::BeginPopupModal("Confirm Action", NULL,
  ...
  // ==========================================
  if (ImGui::CollapsingHeader("Points List & Tools",
                              ImGuiTreeNodeFlags_DefaultOpen)) {
    // --- CREATION BUTTONS ---
    ImGui::Text("Tools:");
    if (ImGui::Button("Free Point")) {
      Point p;
      p.name = "P" + std::to_string(entities.points.size());
      entities.points.push_back(p);
    }
    ImGui::SameLine();
    if (ImGui::Button("Dilate"))
      CreatePoint(entities, OP_DILATE, {0, 0}, 2.0f);
    ImGui::SameLine();
    if (ImGui::Button("Translate"))
      CreatePoint(entities, OP_TRANSLATE, {0, 0, 0});
    ImGui::SameLine();
    if (ImGui::Button("Rotate"))
      CreatePoint(entities, OP_ROTATE_LINE, {0, 0, 0}, 90.0f);

    if (ImGui::Button("Intersect"))
      CreatePoint(entities, OP_INTERSECT_LINES, {0, 0, 0, 0});
    ImGui::SameLine();
    if (ImGui::Button("Project"))
      CreatePoint(entities, OP_PROJECT_LINE, {0, 0, 0});
    ImGui::SameLine();
    if (ImGui::Button("Reflect Pl"))
      CreatePoint(entities, OP_REFLECT_PLANE, {0, 0, 0, 0});

    if (ImGui::Button("Perp Plane"))
      CreatePoint(entities, OP_PERP_TO_PLANE, {0, 0, 0, 0}, 1.0f);
    ImGui::SameLine();
    if (ImGui::Button("Angle Div"))
      CreatePoint(entities, OP_ANGLE_DIV, {0, 0, 0}, 0.5f);
    ...
    for (int i = 0; i < (int)entities.points.size(); i++) {
      ...
      // Name Input
      char buf[64];
      snprintf(buf, 64, "%s", p.name.c_str());
      ...
      if (p.op == OP_FREE) {
        ...
        // If user drags float, we need to propagate updates to children
        // immediately
...
#define UPDATE_PARENT(slot, label)                                             \
  {                                                                            \
    ...
    if (PointSelector(label, &tmp, entities.points)) {                         \
      SetPointParent(entities, i, slot, tmp);                                  \
        ...
        if (p.op == OP_DILATE) {
          if (p.parents.size() < 2)
            p.parents.resize(2, 0);
          UPDATE_PARENT(0, "Target");
          UPDATE_PARENT(1, "Center");
          ImGui::SetNextItemWidth(80);
          if (ImGui::DragFloat("Scale", &p.param, 0.01f))
            PropagateUpdates(entities, i);
        } else if (p.op == OP_TRANSLATE) {
          if (p.parents.size() < 3)
            p.parents.resize(3, 0);
          UPDATE_PARENT(0, "Target");
          UPDATE_PARENT(1, "Start");
          UPDATE_PARENT(2, "End");
        } else if (p.op == OP_ROTATE_LINE) {
          if (p.parents.size() < 3)
            p.parents.resize(3, 0);
          UPDATE_PARENT(0, "Target");
          UPDATE_PARENT(1, "Axis A");
          UPDATE_PARENT(2, "Axis B");
          if (ImGui::DragFloat("Deg", &p.param, 1.0f))
            PropagateUpdates(entities, i);
        } else if (p.op == OP_INTERSECT_LINES) {
          if (p.parents.size() < 4)
            p.parents.resize(4, 0);
          UPDATE_PARENT(0, "L1 A");
          UPDATE_PARENT(1, "L1 B");
          UPDATE_PARENT(2, "L2 A");
          UPDATE_PARENT(3, "L2 B");
        } else if (p.op == OP_PROJECT_LINE) {
          if (p.parents.size() < 3)
            p.parents.resize(3, 0);
          UPDATE_PARENT(0, "Target");
          UPDATE_PARENT(1, "Line A");
          UPDATE_PARENT(2, "Line B");
        } else if (p.op == OP_REFLECT_PLANE) {
          if (p.parents.size() < 4)
            p.parents.resize(4, 0);
          UPDATE_PARENT(0, "Target");
          UPDATE_PARENT(1, "Pln 1");
          UPDATE_PARENT(2, "Pln 2");
          UPDATE_PARENT(3, "Pln 3");
          ...
          UPDATE_PARENT(2, "Pln 2");
          UPDATE_PARENT(3, "Pln 3");
          if (ImGui::DragFloat("Dist", &p.param, 0.1f))
            PropagateUpdates(entities, i);
        } else if (p.op == OP_ANGLE_DIV) {
          if (p.parents.size() < 3)
            p.parents.resize(3, 0);
          UPDATE_PARENT(0, "Start");
          UPDATE_PARENT(1, "Vertex");
          UPDATE_PARENT(2, "End");
          if (ImGui::DragFloat("Ratio", &p.param, 0.01f))
            PropagateUpdates(entities, i);
        }
      }
      ...
      if (ImGui::TreeNode("Style")) {
        ...
        if (p.showLabel)
          EditColor("Text Color", p.textColor);
        ImGui::TreePop();
      }
      ImGui::PopID();
      ImGui::Separator();
    ...
    if (point_to_delete != -1)
      DeletePoint(entities, point_to_delete);
  }
  ...
  if (ImGui::CollapsingHeader("Measurements")) {
    if (ImGui::Button("Add Angle Measure"))
      entities.angles.push_back(AngleMeas());
    ...
    if (remove_ang != -1)
      entities.angles.erase(entities.angles.begin() + remove_ang);
  }
  ...
  if (ImGui::CollapsingHeader("Lines & Rays")) {
    if (ImGui::Button("Add Segment"))
      entities.segments.push_back(Segment());
    ImGui::SameLine();
    if (ImGui::Button("Add Line"))
      entities.lines.push_back(Line());
    ImGui::SameLine();
    if (ImGui::Button("Add Ray"))
      entities.rays.push_back(Ray());
    ...
    for (int i = 0; i < (int)entities.segments.size(); i++) {
      ...
      if (ImGui::TreeNode("Seg", "Segment %d", i)) {
        ...
        EditLineStyle(entities.segments[i].thickness,
                      entities.segments[i].dashLen,
                      entities.segments[i].gapLen);
        if (ImGui::Button("Delete"))
          remove_seg = i;
        ImGui::TreePop();
      }
    ...
    if (remove_seg != -1)
      entities.segments.erase(entities.segments.begin() + remove_seg);
    ...
    for (int i = 0; i < (int)entities.lines.size(); i++) {
      ...
      if (ImGui::TreeNode("Line", "Line %d", i)) {
        ...
        EditLineStyle(entities.lines[i].thickness, entities.lines[i].dashLen,
                      entities.lines[i].gapLen);
        if (ImGui::Button("Delete"))
          remove_line = i;
        ImGui::TreePop();
      }
    ...
    if (remove_line != -1)
      entities.lines.erase(entities.lines.begin() + remove_line);
    ...
    for (int i = 0; i < (int)entities.rays.size(); i++) {
      ...
      if (ImGui::TreeNode("Ray", "Ray %d", i)) {
        ...
        EditLineStyle(entities.rays[i].thickness, entities.rays[i].dashLen,
                      entities.rays[i].gapLen);
        if (ImGui::Button("Delete"))
          remove_ray = i;
        ImGui::TreePop();
      }
    ...
    if (remove_ray != -1)
      entities.rays.erase(entities.rays.begin() + remove_ray);
  }
  ...
  if (ImGui::CollapsingHeader("Surfaces")) {
    if (ImGui::Button("Add Surface"))
      entities.surfaces.push_back(Surface());
    ...
    for (int i = 0; i < (int)entities.surfaces.size(); i++) {
      ...
      if (ImGui::TreeNode("Surf", "Surface %d", i)) {
        ...
        if (ImGui::Button("Add Ref"))
          surf.pointIndices.push_back(0);
        ImGui::SameLine();
        if (surf.pointIndices.size() > 0 && ImGui::Button("Rem Ref"))
          surf.pointIndices.pop_back();
    ...
    if (remove_surf != -1)
      entities.surfaces.erase(entities.surfaces.begin() + remove_surf);
  }
  ...
  if (ImGui::CollapsingHeader("Circles & Planes")) {
    if (ImGui::Button("Add Circle"))
      entities.circles.push_back(Circle());
    ImGui::SameLine();
    if (ImGui::Button("Add Plane"))
      entities.planes.push_back(Plane());
    ...
    for (int i = 0; i < (int)entities.circles.size(); i++) {
      ...
      if (ImGui::TreeNode("Circ", "Circle %d", i)) {
        ...
        EditLineStyle(entities.circles[i].thickness,
                      entities.circles[i].dashLen, entities.circles[i].gapLen);
        if (ImGui::Button("Delete"))
          remove_circ = i;
        ImGui::TreePop();
      }
    ...
    if (remove_circ != -1)
      entities.circles.erase(entities.circles.begin() + remove_circ);
    ...
    if (remove_plane != -1)
      entities.planes.erase(entities.planes.begin() + remove_plane);
  }
  ...
  if (ImGui::CollapsingHeader("3D Shapes")) {
    ...
    for (int i = 0; i < (int)entities.spheres.size(); i++) {
      ...
      if (ImGui::TreeNode("Sph", "Sphere %d", i)) {
        ...
        PointSelector("Radius P", &entities.spheres[i].edgepoint,
                      entities.points);
        EditColor("Body", entities.spheres[i].sphereColor);
        EditColor("Ring", entities.spheres[i].ringColor);
        EditLineStyle(entities.spheres[i].ringThickness,
                      entities.spheres[i].dashLen, entities.spheres[i].gapLen);
        if (ImGui::Button("Delete"))
          remove_sph = i;
        ImGui::TreePop();
      }
    ...
    if (remove_sph != -1)
      entities.spheres.erase(entities.spheres.begin() + remove_sph);
    ...
    for (int i = 0; i < (int)entities.cylinders.size(); i++) {
      ...
      if (ImGui::TreeNode("Cyl", "Cylinder %d", i)) {
        ...
        PointSelector("Edge", &entities.cylinders[i].edgepoint,
                      entities.points);
        ...
        EditLineStyle(entities.cylinders[i].outlineThickness,
                      entities.cylinders[i].dashLen,
                      entities.cylinders[i].gapLen);
        if (ImGui::Button("Delete"))
          remove_cyl = i;
        ImGui::TreePop();
      }
    ...
    if (remove_cyl != -1)
      entities.cylinders.erase(entities.cylinders.begin() + remove_cyl);
    ...
    for (int i = 0; i < (int)entities.cones.size(); i++) {
      ...
      if (ImGui::TreeNode("Cone", "Cone %d", i)) {
        ...
        EditLineStyle(entities.cones[i].outlineThickness,
                      entities.cones[i].dashLen, entities.cones[i].gapLen);
        if (ImGui::Button("Delete"))
          remove_cone = i;
        ImGui::TreePop();
      }
    ...
    if (remove_cone != -1)
      entities.cones.erase(entities.cones.begin() + remove_cone);
  }
...
void RenderScene(Renderer *renderer, Camera *cam,
                 const DrawEntities &entities) {
  if (!renderer || !cam)
    return;
  auto GetPos = [&](int idx) -> glm::vec3 {
    if (idx >= 0 && idx < (int)entities.points.size())
      return entities.points[idx].pos;
    return glm::vec3(0.0f);
  };
  ...
  for (const auto &p : entities.points) {
    ...
    // glm::u8vec4 ringCol = (p.op != OP_FREE) ? glm::u8vec4{0,255,255,255} :
    // glm::u8vec4{0,0,0,0};
    DrawSphere(renderer, cam, p.pos, radiusPoint, 8, 8, p.color, {0, 0, 0, 0},
               0.05f, 0.0f, 0.0f);
    if (p.showLabel) {
      glm::vec3 textPos = p.pos;
      textPos.y += p.visualRadius + entities.globalTextOffset;
      DrawText(renderer, cam, p.name.c_str(), textPos, entities.globalTextSize,
               p.textColor);
    }
  }
  ...
  // 2. Lines & Segments
  for (const auto &l : entities.lines)
    DrawLine(renderer, GetPos(l.point1), GetPos(l.point2), l.color, l.thickness,
             l.dashLen, l.gapLen);
  for (const auto &r : entities.rays)
    DrawRay(renderer, GetPos(r.point1), GetPos(r.point2), r.color, r.thickness,
            r.dashLen, r.gapLen);
  for (const auto &s : entities.segments) {
  ...
  for (const auto &a : entities.angles) {
    ...
    DrawAngle(renderer, cam, GetPos(a.p1), GetPos(a.vertex), GetPos(a.p2),
              a.color, a.radius, a.thickness, entities.globalTextSize,
              entities.globalTextOffset);
  }

  // 4. Other Shapes
  for (const auto &surf : entities.surfaces) {
    if (surf.pointIndices.size() < 3)
      continue;
    std::vector<glm::vec3> polyPoints;
    polyPoints.reserve(surf.pointIndices.size());
    for (int idx : surf.pointIndices)
      polyPoints.push_back(GetPos(idx));
    DrawSurface(renderer, polyPoints.data(), (int)polyPoints.size(),
                surf.surfaceColor, surf.borderColor, surf.borderThickness,
                surf.dashLen, surf.gapLen);
  }
  for (const auto &p : entities.planes)
    DrawPlane(renderer, GetPos(p.point1), GetPos(p.point2), GetPos(p.point3),
              p.color);
  for (const auto &c : entities.circles)
    DrawCircle(renderer, GetPos(c.midpoint), GetPos(c.edgepoint),
               GetPos(c.normpoint), c.steps, c.surfaceColor, c.borderColor,
               c.thickness, c.dashLen, c.gapLen);
  for (const auto &s : entities.spheres)
    DrawSphere(renderer, cam, GetPos(s.midpoint), GetPos(s.edgepoint),
               s.sectorCount, s.stackCount, s.sphereColor, s.ringColor,
               s.ringThickness, s.dashLen, s.gapLen);
  for (const auto &c : entities.cylinders)
    DrawCylinder(renderer, cam, GetPos(c.midpoint), GetPos(c.toppoint),
                 GetPos(c.edgepoint), c.sectorCount, c.cylinderColor,
                 c.outlineColor, c.outlineThickness, c.dashLen, c.gapLen);
  for (const auto &c : entities.cones)
    DrawCone(renderer, cam, GetPos(c.midpoint), GetPos(c.toppoint),
             GetPos(c.edgepoint), c.sectorCount, c.coneColor, c.outlineColor,
